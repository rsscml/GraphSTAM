...
class graphmodel():
    ...
    def create_train_test_dataset(self, df):
        ...
        all_snap_periods_list = sorted(df[df[self.time_index_col] <= test_cutoff][self.time_index_col].unique(), reverse=False)
        test_snap_period_list = sorted(df[(df[self.time_index_col] > self.train_till) & (df[self.time_index_col] <= test_cutoff)][self.time_index_col].unique(), reverse=False)

        # for each split create graph dataset iterator
        print("gather all snapshot graphs...")
        ...
        if self.subgraph_sample_col is not None:
            ...
            # sample snapshot graphs
            snapshot_list = Parallel(n_jobs=self.PARALLEL_DATA_JOBS, batch_size=self.PARALLEL_DATA_JOBS_BATCHSIZE)(delayed(parallel_snapshot_graphs)(df, period) for period in all_snap_periods_list)
    ...
    def create_train_test_dataset_original(self, df):
