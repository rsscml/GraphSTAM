...
class graphmodel:
    ...
    def create_snapshot_graph(self, df_snap, period):
        ...
        data['key_aggregation_status'].x = torch.tensor(np.select(kl_conditions, kl_choices).reshape(-1, 1), dtype=torch.int32)
        ...
        for col in self.static_cat_col_list:
            ...
            for value in col_unique_values:
                # get subset of all nodes with common col value
                nodes = df_snap[(df_snap[col] == value) & (df_snap['key_level'] == self.covar_key_level)][self.id_col].to_numpy()
                # Build all combinations of connected nodes
                permutations = list(itertools.combinations(nodes, 2))
                edges_source = [e[0] for e in permutations]
                edges_target = [e[1] for e in permutations]
            ...
            # edge names
            edge_name = (self.target_col, 'related_by_{}_at_{}'.format(col,self.covar_key_level), self.target_col)
            rev_edge_name = (self.target_col, 'rev_related_by_{}_at_{}'.format(col, self.covar_key_level), self.target_col)
            # add edges to Data()
            edges = np.concatenate(fwd_edges_stack, axis=0)
            rev_edges = np.concatenate(rev_edges_stack, axis=0)
            data[edge_name].edge_index = torch.tensor(edges.transpose(), dtype=torch.long)
    ...
    def train(self,
        ...
        for epoch in range(max_epochs):
            ...
            if stop_training_criteria == 'loss':
                ...
                save_condition = ((val_metric_hist[epoch] == np.min(val_metric_hist)) and (-delta > min_delta)) or (epoch == 0)
