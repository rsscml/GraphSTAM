...
class graphmodel():
    ...
    def create_infer_dataset(self, df, infer_till):
        ...
        infer_snap_period = sorted(infer_df[self.time_index_col].unique(), reverse=False)[-1]
        infer_snap = infer_df[infer_df[self.time_index_col] == infer_snap_period].reset_index(drop=True)
        infer_graph = self.create_snapshot_graph(infer_snap, infer_snap_period)

        del infer_snap
        gc.collect()

        return infer_df, infer_graph

    def get_metadata(self, batch):
        return batch.metadata()
    
    def show_batch_statistics(self, batch):
    ...
    def build(self,
              ...
              device='cpu'):
        # build config
        self.build_config = {'layer_type': layer_type,
                             'model_dim': model_dim,
                             'num_layers': num_layers,
                             'heads': heads,
    ...
    def train(self, 
        ...
        # create lags & additional features
        ...
        # get all timestamps at which to construct snapshot graphs
        def get_snapshot_periods(df):
            train_snap_periods = sorted(df[df[self.time_index_col] <= self.train_till][self.time_index_col].unique(), reverse=False)
            test_snap_periods = sorted(df[(df[self.time_index_col] > self.train_till) & (df[self.time_index_col] <= self.test_till)][self.time_index_col].unique(), reverse=False)

            # restrict samples for very large datasets based on interleaving
            train_snap_periods = train_snap_periods[int(self.max_target_lags - 1):]

            if self.interleave > 1:
                train_snap_periods = train_snap_periods[0::self.interleave] + [train_snap_periods[-1]]

            print("train samples: {}".format(len(train_snap_periods)))
            print("test samples: {}".format(len(test_snap_periods)))

            return train_snap_periods, test_snap_periods

        # create snapshot graph
        def create_graph(df, period):
        ...
        # create feature df & train/test snap periods list
        feature_df = create_features(self.onetime_prep_df)
        train_periods, test_periods = get_snapshot_periods(feature_df)

        # shuffle train/test periods
        random.shuffle(train_periods)
        random.shuffle(test_periods)

        # build & init model
        def init_model():
            period = random.choice(train_periods)
            sample_batch = create_graph(feature_df, period)
            # get metadata
            metadata = self.get_metadata(sample_batch)
            self.forecast_quantiles = self.build_config['forecast_quantiles']
            # target device to train on ['cuda','cpu']
            self.device = torch.device(self.build_config['device'])

            # build model
            self.model = STGNN(hidden_channels=self.build_config['model_dim'],
                               metadata=metadata,
                               target_node=self.target_col,
                               time_steps=self.fh,
                               n_quantiles=len(self.forecast_quantiles),
                               num_layers=self.build_config['num_layers'],
                               heads=self.build_config['heads'],
                               dropout=self.build_config['dropout'],
                               skip_connection=self.build_config['skip_connection'],
                               layer_type=self.build_config['layer_type'])
        ...
        for epoch in range(max_epochs):
            ...
            # shuffle train/test periods for each epoch
            random.shuffle(train_periods)
            random.shuffle(test_periods)
