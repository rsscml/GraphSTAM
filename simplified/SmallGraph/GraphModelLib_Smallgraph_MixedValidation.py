    ...
    def forward(self, x_dict, edge_index_dict):
        ...
        if not self.is_output_layer:
            for node_type, norm in self.norm_dict.items():
                x = norm(torch.nn.functional.selu(self.dropout(x_dict[node_type])))
                x_dict[node_type] = x
            #x_dict[self.target_node_type] = self.project_lin(x_dict[self.target_node_type])
        else:
            x_dict[self.target_node_type] = torch.nn.functional.selu(x_dict[self.target_node_type])
...
class HeteroGraphSAGE(torch.nn.Module):
    ...
    def forward(self, x_dict, edge_index_dict):
        ...
        for node_type, x in x_dict.items():
            x_dict[node_type] = torch.nn.functional.selu(self.node_proj[node_type](x))
        """
...
class graphmodel():
    ...
    def create_train_test_dataset(self, df):
        ...
        snap_periods_list = sorted(df[df[self.time_index_col] <= self.test_till][self.time_index_col].unique(),
                                   reverse=False)
        ...
        if self.subgraph_sample_col is not None:
            ...
            if self.subgraph_sample_size > 0:
                ...
                for i in range(0, len(all_subgraph_col_values), int(self.subgraph_sample_size)):
                    ...
                    print("  gathering for subgraph_col_values: ",
                          all_subgraph_col_values[i:i + self.subgraph_sample_size])
                    sample_snapshot_list = Parallel(n_jobs=self.PARALLEL_DATA_JOBS,
                                                    batch_size=self.PARALLEL_DATA_JOBS_BATCHSIZE)(
                        delayed(parallel_snapshot_graphs)(df_sample, period) for period in snap_periods_list)
                    ...
                    train_snapshots += [sample_snapshot_list[i] for i in range(num_nonrecent_snapshots) if
                                        i not in val_random_select_indices]
                ...
                sample_snapshot_list = Parallel(n_jobs=self.PARALLEL_DATA_JOBS,
                                                batch_size=self.PARALLEL_DATA_JOBS_BATCHSIZE)(
                    delayed(parallel_snapshot_graphs)(df, period) for period in snap_periods_list)
                ...
                nonrecent_val_snapshots = [list(itertools.chain.from_iterable(snapshot_list))[i] for i in
                                           val_random_select_indices]
                test_snapshots = recent_val_snapshots + nonrecent_val_snapshots
                train_snapshots = [list(itertools.chain.from_iterable(snapshot_list))[i] for i in
                                   range(num_nonrecent_snapshots) if i not in val_random_select_indices]
            ...
            test_snapshots = recent_val_snapshots + nonrecent_val_snapshots
            train_snapshots = [snapshot_list[i] for i in range(num_nonrecent_snapshots) if
                               i not in val_random_select_indices]
