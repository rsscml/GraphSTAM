...
class STGNN(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.prior_dims = (state_dim * 1 + state_dim * state_dim)
        self.transition_dims = (state_dim * state_dim + state_dim * state_dim)
        self.observation_dims = (state_dim * obs_dim + obs_dim * obs_dim)
        ...
        if self.model_option == "BASIC":
            ...
            self.project_prior = Linear(self.hidden_channels, self.prior_dims)
            self.project_transition = Linear(self.hidden_channels, self.transition_dims)
            self.project_observation = Linear(self.hidden_channels, self.observation_dims)
    ...
    def forward(self, x_dict, edge_index_dict):
        ...
        if self.model_option == "BASIC":
            ...
            prior_mean = torch.reshape(prior_out[:, :self.state_dim], (-1, self.state_dim, 1))
            prior_covariance = torch.reshape(F.softplus(prior_out[:, self.state_dim:]),
            ...
            transition_out = torch.reshape(transition_out, (-1, self.transition_dims))
            transition_matrix = torch.reshape(transition_out[:, :self.state_dim * self.state_dim],
                                              (-1, self.state_dim, self.state_dim))
            transition_covariance = torch.reshape(F.softplus(transition_out[:, self.state_dim * self.state_dim:]),
                                                  (-1, self.state_dim, self.state_dim))
            # ensure positive definite cov matrix
            ...
            observation_out = torch.reshape(observation_out, (-1, self.observation_dims))
            observation_matrix = torch.reshape(observation_out[:, :self.state_dim * self.obs_dim],
                                               (-1, self.state_dim, self.obs_dim))
            observation_covariance = torch.reshape(F.softplus(observation_out[:, self.state_dim * self.obs_dim:]),
...
class graphmodel():
    ...
    def build(self,
        ...
        with torch.no_grad():
            ...
            prior_mu, prior_sigma, trans_mu, trans_sigma, obs_mu, obs_sigma = self.model(sample_batch.x_dict, sample_batch.edge_index_dict)
    ...
    def train(self, 
        ...
        def train_fn():
            ...
            for i, batch in enumerate(self.train_dataset):
                ...
                prior_mu, prior_cov, trans_mu, trans_cov, obs_mu, obs_cov = self.model(batch.x_dict, batch.edge_index_dict)

                # compute loss masking out N/A targets -- last snapshot
                log_prob = lgssmlogprob(x=batch[self.target_col].y,
                                        z_prior_mean=prior_mu,
                                        z_prior_covariance=prior_cov,
                                        transition_matrix=trans_mu,
                                        transition_covariance=trans_cov,
                                        observation_matrix=obs_mu,
                ...
                log_loss = torch.mean(log_prob * mask * wt)

                # normalize loss to account for batch accumulation
                if self.grad_accum:
                    log_loss = log_loss / self.accum_iter
                    log_loss.backward()
                    # weights update
        ...
        def test_fn():
            ...
            with torch.no_grad():
                for i, batch in enumerate(self.test_dataset):
                    batch_size = batch.num_graphs
                    batch = batch.to(self.device)
                    prior_mu, prior_cov, trans_mu, trans_cov, obs_mu, obs_cov = self.model(batch.x_dict,
    ...
    def infer(self, infer_start, infer_end, select_quantile, num_samples=100):
        ...
        for i, t in enumerate(infer_periods):
            ...
            if self.include_rolling_features:
                self.temporal_unknown_num_col_list = list(
                    set(self.temporal_unknown_num_col_list) - set(self.rolling_stat_cols))

            if not self.categorical_onehot_encoding:
                self.temporal_known_num_col_list = list(
                    set(self.temporal_known_num_col_list) - set(self.label_encoded_col_list))
                self.temporal_unknown_num_col_list = list(
                    set(self.temporal_unknown_num_col_list) - set(self.label_encoded_col_list))
            ...
            output_arr = output[0]
            output_arr = output_arr.cpu().numpy()  # (batch, num_samples)

            # quantile selection
            min_qtile, max_qtile = min(self.forecast_quantiles), max(self.forecast_quantiles)

            if self.loss_type == 'Quantile':
                output_arr = np.quantile(output_arr, q=select_quantile, axis=1, keepdims=True)
            else:
                output_arr = np.mean(output_arr, axis=1, keepdims=True)
